(function () {
  ...
  function buildSubmission(form, selected) {
    ...
    const mediaUrls = form.mediaUrls || "Attached below.";
    const installCommand = form.installCommand || summary.install_command || INSTALL_COMMAND;
    const currentPlugins = Array.isArray(summary.plugins) && summary.plugins.length
      ? summary.plugins.map(function (plugin) {
          const label = plugin.label || plugin.name || "Plugin";
    ...
    return [
      ...
      pitch,
      "",
      currentPlugins ? "**What it includes**" : null,
      currentPlugins || null,
      currentPlugins ? "" : null,
      currentPlugins ? "**Plugin enablement**" : null,
      currentPlugins ? "Plugin enablement is managed locally through `hermes-dashboard-plugins`. It adds the `/plugins` dashboard page and lets installed dashboard plugins be enabled or disabled without editing Hermes core." : null,
      "",
      "**Screenshots / video**",
      mediaUrls,
      "",
      ...
      "",
      "**Notes**",
      form.notes || "MVP does not auto-post to Discord. It generates a review-ready submission and keeps certification claims honest.\n\nDraft generated with `hermes-hackathon-hub` from this repository.",
